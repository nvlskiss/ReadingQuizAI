...
class NotebookDatabase:
    ...
    def get_notebook_sets(self, notebook_id: int) -> Optional[Dict]:
        with self._connect() as conn:
            ...
            elif notebook["question_setting_id"] and notebook["quiz_id"]:
                settings = self._fetch_settings(conn, notebook["question_setting_id"])
                questions = self._fetch_questions(conn, notebook["quiz_id"])
                sets.append(
                    {
                        "set_id": None,
                        "set_name": "Set 1",
    ...
    def _ensure_notebook_sets_initialized(self, conn, notebook_id: int):
        ...
        cursor.execute(
            "SELECT question_setting_id, quiz_id FROM notebook WHERE id = ?",
            (notebook_id,),
        )
        notebook_row = cursor.fetchone()
        if not notebook_row:
